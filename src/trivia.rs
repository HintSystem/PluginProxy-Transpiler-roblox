...
use full_moon::ast::luau::{
    ElseIfExpression, GenericDeclaration, GenericDeclarationParameter, GenericParameterInfo, IfExpression, IndexedTypeInfo,
    InterpolatedString, InterpolatedStringSegment, TypeArgument, TypeAssertion, TypeDeclaration, TypeField, TypeFieldKey,
    TypeInfo, TypeSpecifier,
};
use full_moon::ast::{
    punctuated::Punctuated, span::ContainedSpan, BinOp, Call, Expression, FunctionArgs, FunctionBody, FunctionCall, FunctionName,
    Index, LastStmt, MethodCall, Parameter, Prefix, Return, Stmt, Suffix, TableConstructor, UnOp, Var, VarExpression,
};
...
impl UpdateTrivia for TokenReference {
    fn update_trivia(&self, leading_trivia: FormatTriviaType, trailing_trivia: FormatTriviaType) -> Self {
        ...
        let added_trailing_trivia = match trailing_trivia {
            FormatTriviaType::Append(trivia) => {
                let mut current: Vec<Token> = self.trailing_trivia().map(|x| x.to_owned()).collect();
                current.extend(trivia);
                current
            }
            FormatTriviaType::Replace(trivia) => trivia,
...
define_update_trivia!(Call, |this, leading, trailing| {
    match this {
        Call::AnonymousCall(function_args) => Call::AnonymousCall(function_args.update_trivia(leading, trailing)),
        Call::MethodCall(method_call) => Call::MethodCall(method_call.update_trivia(leading, trailing)),
        other => panic!("unknown node {:?}", other),
    }
});
...
define_update_leading_trivia!(Expression, |this, leading| {
    match this {
        ...
        Expression::Function(boxed) => {
            let (token, function_body) = &**boxed;
            Expression::Function(Box::new((token.update_leading_trivia(leading), function_body.to_owned())))
        }
        Expression::FunctionCall(function_call) => Expression::FunctionCall(function_call.update_leading_trivia(leading)),
        Expression::Number(token_reference) => Expression::Number(token_reference.update_leading_trivia(leading)),
        Expression::String(token_reference) => Expression::String(token_reference.update_leading_trivia(leading)),
        Expression::Symbol(token_reference) => Expression::Symbol(token_reference.update_leading_trivia(leading)),
        ...
        #[cfg(feature = "luau")]
        Expression::IfExpression(if_expression) => Expression::IfExpression(if_expression.update_leading_trivia(leading)),
        #[cfg(feature = "luau")]
...
define_update_trailing_trivia!(Expression, |this, trailing| {
    match this {
        Expression::Function(boxed) => {
            let (token, function_body) = &**boxed;
            Expression::Function(Box::new((token.to_owned(), function_body.update_trailing_trivia(trailing))))
        }
        Expression::FunctionCall(function_call) => Expression::FunctionCall(function_call.update_trailing_trivia(trailing)),
        Expression::Number(token_reference) => Expression::Number(token_reference.update_trailing_trivia(trailing)),
        Expression::String(token_reference) => Expression::String(token_reference.update_trailing_trivia(trailing)),
        Expression::Symbol(token_reference) => Expression::Symbol(token_reference.update_trailing_trivia(trailing)),
        Expression::TableConstructor(table_constructor) => {
        ...
        #[cfg(feature = "luau")]
        Expression::IfExpression(if_expression) => Expression::IfExpression(if_expression.update_trailing_trivia(trailing)),
        #[cfg(feature = "luau")]
        Expression::InterpolatedString(interpolated_string) => {
...
define_update_trivia!(FunctionArgs, |this, leading, trailing| {
    match this {
        ...
        FunctionArgs::String(token_reference) => FunctionArgs::String(token_reference.update_trivia(leading, trailing)),
...
define_update_trivia!(FunctionName, |this, leading, trailing| {
    if let Some(method_name) = this.method_name() {
        ...
        let names = this.names().update_leading_trivia(leading).update_trailing_trivia(trailing);
...
define_update_trivia!(Return, |this, leading, trailing| {
    if this.returns().is_empty() {
        return this.to_owned().with_token(this.token().update_trivia(leading, trailing));
    } else {
        return this
            .to_owned()
...
define_update_leading_trivia!(Prefix, |this, leading| {
    match this {
        Prefix::Name(token_reference) => Prefix::Name(token_reference.update_leading_trivia(leading)),
        Prefix::Expression(expression) => Prefix::Expression(Box::new(expression.update_leading_trivia(leading))),
        other => panic!("unknown node {:?}", other),
    }
});
...
define_update_trailing_trivia!(Prefix, |this, trailing| {
    match this {
        Prefix::Name(token_reference) => Prefix::Name(token_reference.update_trailing_trivia(trailing)),
        Prefix::Expression(expression) => Prefix::Expression(Box::new(expression.update_trailing_trivia(trailing))),
        other => panic!("unknown node {:?}", other),
    }
});
...
define_update_trivia!(Stmt, |this, leading, trailing| {
    match this {
        Stmt::Assignment(assignment) => Stmt::Assignment(assignment.update_trivia(leading, trailing)),
        Stmt::LocalAssignment(local_assignment) => Stmt::LocalAssignment(local_assignment.update_trivia(leading, trailing)),
        Stmt::FunctionCall(function_call) => Stmt::FunctionCall(function_call.update_trivia(leading, trailing)),
        Stmt::Repeat(repeat_block) => {
            ...
            Stmt::Repeat(repeat_block.to_owned().with_repeat_token(repeat_token).with_until(until))
        }
        Stmt::Do(stmt) => {
            let do_token = stmt.do_token().update_leading_trivia(leading);
            let end_token = stmt.end_token().update_trailing_trivia(trailing);
            Stmt::Do(stmt.to_owned().with_do_token(do_token).with_end_token(end_token))
        }
        Stmt::GenericFor(stmt) => {
            let for_token = stmt.for_token().update_leading_trivia(leading);
            let end_token = stmt.end_token().update_trailing_trivia(trailing);
            Stmt::GenericFor(stmt.to_owned().with_for_token(for_token).with_end_token(end_token))
        }
        ...
        Stmt::FunctionDeclaration(stmt) => {
            ...
            Stmt::FunctionDeclaration(stmt.to_owned().with_function_token(function_token).with_body(body))
        }
        Stmt::LocalFunction(stmt) => {
            let local_token = stmt.local_token().update_leading_trivia(leading);
            let end_token = stmt.body().end_token().update_trailing_trivia(trailing);
            let body = stmt.body().to_owned().with_end_token(end_token);
            Stmt::LocalFunction(stmt.to_owned().with_local_token(local_token).with_body(body))
        }
        Stmt::NumericFor(stmt) => {
            let for_token = stmt.for_token().update_leading_trivia(leading);
            let end_token = stmt.end_token().update_trailing_trivia(trailing);
            Stmt::NumericFor(stmt.to_owned().with_for_token(for_token).with_end_token(end_token))
        }
        Stmt::While(stmt) => {
            let while_token = stmt.while_token().update_leading_trivia(leading);
            let end_token = stmt.end_token().update_trailing_trivia(trailing);
            Stmt::While(stmt.to_owned().with_while_token(while_token).with_end_token(end_token))
        }
...
define_update_trivia!(TableConstructor, |this, leading, trailing| {
    this.to_owned().with_braces(this.braces().update_trivia(leading, trailing))
});
...
define_update_leading_trivia!(Var, |this, leading| {
    match this {
        ...
        Var::Expression(var_expresion) => Var::Expression(Box::new(var_expresion.update_leading_trivia(leading))),
        other => panic!("unknown node {:?}", other),
    }
});
...
define_update_trailing_trivia!(Var, |this, trailing| {
    match this {
        ...
        Var::Expression(var_expression) => Var::Expression(Box::new(var_expression.update_trailing_trivia(trailing))),
        other => panic!("unknown node {:?}", other),
    }
});
...
define_update_trivia!(TypeInfo, |this, leading, trailing| {
    match this {
        ...
        TypeInfo::Basic(token_reference) => TypeInfo::Basic(token_reference.update_trivia(leading, trailing)),
        ...
        TypeInfo::Callback {
            ...
            let (generics, parentheses) = if let Some(generics) = generics {
                (Some(generics.update_leading_trivia(leading)), parentheses.to_owned())
            } else {
        ...
        TypeInfo::Generic { base, arrows, generics } => TypeInfo::Generic {
        ...
        TypeInfo::Intersection { left, ampersand, right } => TypeInfo::Intersection {
        ...
        TypeInfo::Optional { base, question_mark } => TypeInfo::Optional {
        ...
        TypeInfo::Variadic { ellipsis, type_info } => TypeInfo::Variadic {
...
define_update_trailing_trivia!(IndexedTypeInfo, |this, trailing| {
    match this {
        IndexedTypeInfo::Basic(token_reference) => IndexedTypeInfo::Basic(token_reference.update_trailing_trivia(trailing)),
        IndexedTypeInfo::Generic { base, arrows, generics } => IndexedTypeInfo::Generic {
            base: base.to_owned(),
            arrows: arrows.update_trailing_trivia(trailing),
            generics: generics.to_owned(),
        },
...
define_update_leading_trivia!(TypeField, |this, leading| {
    this.to_owned().with_key(this.key().update_leading_trivia(leading))
});

#[cfg(feature = "luau")]
define_update_trailing_trivia!(TypeField, |this, trailing| {
    this.to_owned().with_value(this.value().update_trailing_trivia(trailing))
});
...
define_update_leading_trivia!(GenericDeclaration, |this, leading| {
    this.to_owned().with_arrows(this.arrows().update_leading_trivia(leading))
});

#[cfg(feature = "luau")]
define_update_leading_trivia!(GenericDeclarationParameter, |this, leading| {
    let parameter_info = match this.parameter() {
        GenericParameterInfo::Name(token) => GenericParameterInfo::Name(token.update_leading_trivia(leading)),
...
define_update_trailing_trivia!(GenericDeclarationParameter, |this, trailing| {
    if let Some(default_type) = this.default_type() {
        ...
        let parameter_info = match this.parameter() {
            GenericParameterInfo::Name(token) => GenericParameterInfo::Name(token.update_trailing_trivia(trailing)),
            GenericParameterInfo::Variadic { name, ellipsis } => GenericParameterInfo::Variadic {
                name: name.to_owned(),
                ellipsis: ellipsis.update_trailing_trivia(trailing),
            },
...
define_update_leading_trivia!(IfExpression, |this, leading| {
    this.to_owned().with_if_token(this.if_token().update_leading_trivia(leading))
});
